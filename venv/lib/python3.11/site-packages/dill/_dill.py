...
def _create_capsule(pointer, name, context, destructor):
    ...
    if attr_found:
        ...
        warnings.warn('Creating a new PyCapsule %s for a C data structure that may not be present in memory. Segmentation faults or other memory errors are possible.' % (
...
def save_type(pickler, obj, postproc_list=None):
    if obj in _typemap:
        ...
        if not obj._field_defaults:
            ...
            pickler.save_reduce(_create_namedtuple, (obj.__name__,
                                obj._fields, obj.__module__, defaults), obj=obj)
        ...
        if not _byref and not obj_recursive and incorrectly_named:  # not a function, but the name was held over
           ...
           # print ("%s\n%s" % (obj.__bases__, obj.__dict__))
            ...
            if incorrectly_named:
                warnings.warn('Cannot locate reference to %r.' %
                              (obj,), PicklingWarning)
            if obj_recursive:
                warnings.warn('Cannot pickle %r: %s.%s has recursive self-references that trigger a RecursionError.' %
                              (obj, obj.__module__, obj_name), PicklingWarning)
           # print (obj.__dict__)
           # print ("%s\n%s" % (type(obj), obj.__name__))
           # print ("%s\n%s" % (obj.__bases__, obj.__dict__))
...
if HAS_CTYPES and hasattr(ctypes, 'pythonapi'):
    ...
    def save_capsule(pickler, obj):
        ...
        warnings.warn('Pickling a PyCapsule (%s) does not pickle any C data structures and could cause segmentation faults or other memory errors when unpickling.' % (
            name,), PicklingWarning)
