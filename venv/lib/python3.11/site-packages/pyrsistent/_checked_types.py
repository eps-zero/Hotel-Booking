...
class InvariantException(Exception):
    ...
    def __init__(self, error_codes=(), missing_fields=(), *args, **kwargs):
        self.invariant_errors = tuple(
            e() if callable(e) else e for e in error_codes)
...
def maybe_parse_user_type(t):
    ...
    is_preserved = isinstance(t, type) and issubclass(
        t, _preserved_iterable_types)
        ...
        for e in it:
            if not any(isinstance(e, get_type(t)) for t in expected_types):
                ...
                raise exception_type(
                    source_class, expected_types, actual_type, e, msg)
    ...
    class Evolver(PythonPVector.Evolver):
        ...
        def persistent(self):
            ...
            if self.is_dirty() or (self._destination_class != type(self._orig_pvector)):
                pv = super(CheckedPVector.Evolver,
                           self).persistent().extend(self._extra_tail)
    ...
    class Evolver(PMap._Evolver):
        ...
        def set(self, key, value):
            _check_types([key], self._destination_class._checked_key_types,
                         self._destination_class, CheckedKeyTypeError)
            _check_types(
                [value], self._destination_class._checked_value_types, self._destination_class)
