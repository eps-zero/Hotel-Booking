...
def set_fields(dct, bases, name):
    dct[name] = dict(sum([list(b.__dict__.get(name, {}).items())
                     for b in bases], []))
...
def check_type(destination_cls, field, name, value):
    if field.type and not any(isinstance(value, get_type(t)) for t in field.type):
        ...
        raise PTypeError(destination_cls, name,
                         field.type, actual_type, message)
...
class _PField(object):
    __slots__ = ('type', 'invariant', 'initial',
                 'mandatory', '_factory', 'serializer')
...
PFIELD_NO_TYPE = ()
def PFIELD_NO_INVARIANT(_): return (True, None)
def PFIELD_NO_FACTORY(x): return x


PFIELD_NO_INITIAL = object()
def PFIELD_NO_SERIALIZER(_, value): return value
...
def field(type=PFIELD_NO_TYPE, invariant=PFIELD_NO_INVARIANT, initial=PFIELD_NO_INITIAL,
    ...
    invariant_function = wrap_invariant(
        invariant) if invariant != PFIELD_NO_INVARIANT and callable(invariant) else invariant
...
def _check_field_parameters(field):
    ...
    if field.initial is not PFIELD_NO_INITIAL and \
        ...
        raise TypeError(
            'Initial has invalid type {0}'.format(type(field.initial)))
