    ...
    class Evolver(object):
        ...
        def __setitem__(self, index, val):
            ...
            if 0 <= index < self._count:
                ...
                if node:
                    ...
                    self._root = self._do_set(
                        self._shift, self._root, index, val)
        ...
        def _do_set(self, level, node, i, val):
            ...
            if level == 0:
                ...
                ret[sub_index] = self._do_set(
                    level - SHIFT, node[sub_index], i, val)
        ...
        def __delitem__(self, key):
            if self._orig_pvector:
                ...
                l = PythonPVector(self._count, self._shift,
                                  self._root, self._tail).tolist()
        ...
        def persistent(self):
            ...
            if self.is_dirty():
                result = PythonPVector(
                    self._count, self._shift, self._root, self._tail).extend(self._extra_tail)
                self._reset(result)
