...
class PMap(object):
    ...
    def update_with(self, update_fn, *maps):
        ...
        for map in maps:
            for key, value in map.items():
                evolver.set(key, update_fn(
                    evolver[key], value) if key in evolver else value)
    ...
    class _Evolver(object):
        ...
        def set(self, key, val):
            ...
            reallocation_required = len(
                self._buckets_evolver) < 0.67 * self._size
            if bucket:
                for k, v in bucket:
                    if k == key:
                        if v is not val:
                            new_bucket = [(k2, v2) if k2 != k else (
                                k2, val) for k2, v2 in bucket]
