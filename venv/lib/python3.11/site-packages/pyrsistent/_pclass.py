...
class PClassMeta(type):
    def __new__(mcs, name, bases, dct):
        ...
        dct['__slots__'] = ('_pclass_frozen',) + \
            tuple(key for key in dct['_pclass_fields'])
...
class PClass(CheckedType, metaclass=PClassMeta):
    ...
    def __new__(cls, **kwargs):    # Support *args?
            ...
            if name in kwargs:
                if factory_fields is None or name in factory_fields:
                    if is_field_ignore_extra_complaint(PClass, field, ignore_extra):
                        value = field.factory(
                            kwargs[name], ignore_extra=ignore_extra)
                    else:
                        value = field.factory(kwargs[name])
                else:
                    value = kwargs[name]
                _check_and_set_attr(cls, field, name, value,
        ...
        if invariant_errors or missing_fields:
            raise InvariantException(tuple(invariant_errors), tuple(
                missing_fields), 'Field invariant failed')
