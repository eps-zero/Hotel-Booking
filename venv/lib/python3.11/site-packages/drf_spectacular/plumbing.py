...
if hasattr(types, 'UnionType'):
    UNION_TYPES: Tuple[Any, ...] = (
        typing.Union, types.UnionType)  # type: ignore
else:
...
if sys.version_info >= (3, 8):
    ...
    from typing import _TypedDictMeta as _PyTypedDictMeta
...
try:
    ...
    # type: ignore[attr-defined]
    from typing_extensions import _TypedDictMeta as _PxTypedDictMeta
...
if sys.version_info >= (3, 8):
    CACHED_PROPERTY_FUNCS = (functools.cached_property,
...
def is_list_serializer_customized(obj) -> bool:
    return (
        ...
        # type: ignore
        and get_class(get_list_serializer(obj)).to_representation
...
def get_lib_doc_excludes():
    ...
    return [
        ...
        *[getattr(serializers, c)
          for c in dir(serializers) if c.endswith('Serializer')],
        *[getattr(viewsets, c)
          for c in dir(viewsets) if c.endswith('ViewSet')],
        *[getattr(generics, c)
          for c in dir(generics) if c.endswith('APIView')],
...
def build_choice_field(field):
    ...
    choices = list(OrderedDict.fromkeys(field.choices))
    ...
    if spectacular_settings.ENUM_GENERATE_CHOICE_DESCRIPTION:
        schema['description'] = build_choice_description_list(
            field.choices.items())
...
def _follow_field_source(model, path: List[str]):
    ...
    if len(path) == 1:
        ...
        if isinstance(field_or_property, property):
            ...
            # type: ignore # o2o & foreign forward
            return field_or_property.field.target_field
        else:
...
class ComponentRegistry:
    ...
    def __contains__(self, component):
        ...
        query_class = query_obj if inspect.isclass(
            query_obj) else query_obj.__class__
        registry_class = query_obj if inspect.isclass(
            registry_obj) else registry_obj.__class__
    ...
    def build(self, extra_components) -> dict:
        ...
        return {
            type: {name: output[type][name]
                   for name in sorted(output[type].keys())}
    ...
    for name, choices in spectacular_settings.ENUM_NAME_OVERRIDES.items():
        ...
        for choice in choices:
            ...
            if isinstance(choice, str) or choice is None:
                ...
                # categorized nested choices
                normalized_choices.extend(choice[1])
            else:
        ...
        hashable_values = [value for value,
                           _ in normalized_choices if value not in ['', None]]
...
def resolve_django_path_parameter(path_regex, variable, available_formats):
    ...
    for match in _PATH_PARAMETER_COMPONENT_RE.finditer(path_regex):
        converter, parameter = match.group(
            'converter'), match.group('parameter')
        enum_values = None
        ...
        if converter in spectacular_settings.PATH_CONVERTER_OVERRIDES:
            ...
            schema['pattern'] = anchor_pattern(
                registered_converters[converter].regex)
        else:
            error(
...
def modify_for_versioning(patterns, method, path, view, requested_version):
    ...
    if issubclass(view.versioning_class, versioning.URLPathVersioning):
        ...
        path = uritemplate.partial(
            path, var_dict={version_param: requested_version})
...
def modify_media_types_for_versioning(view, media_types: List[str]) -> List[str]:
    ...
    version = media_type.params.get(
        view.versioning_class.version_param)  # type: ignore
...
def sanitize_result_object(result):
    ...
    for operation_id, paths in operations.items():
        ...
        for idx, (path, method) in enumerate(sorted(paths)[1:], start=2):
            suffix = str(
                idx) if spectacular_settings.CAMELIZE_NAMES else f'_{idx}'
...
def sanitize_specification_extensions(extensions):
    ...
    for key, value in extensions.items():
        if not re.match(r'^x-', key):
            warn(
                f'invalid extension {key!r}. vendor extensions must start with "x-"')
        else:
            output[key] = value
    return output
...
def camelize_operation(path, operation):
    ...
    operation['operationId'] = inflection.camelize(
...
def resolve_type_hint(hint):
    ...
    if origin is None and is_basic_type(hint, allow_none=False):
        ...
        if get_type_hints(hint):
            properties = {k: resolve_type_hint(
                v) for k, v in get_type_hints(hint).items()}
        else:
            properties = {k: build_basic_type(
                OpenApiTypes.ANY) for k in hint._fields}
        ...
        return build_array_type(
            resolve_type_hint(args[0]) if args else build_basic_type(
                OpenApiTypes.ANY)
        )
...
def build_listed_example_value(value: Any, paginator, direction):
    ...
    try:
        return {
            field_name: [
                value] if field_schema is sentinel else field_schema['example']
