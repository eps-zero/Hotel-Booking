...
class AutoSchema(ViewInspector):
        ...
        for parameter in self.get_override_parameters():
            if isinstance(parameter, OpenApiParameter):
                ...
                if is_basic_type(parameter.type):
                    ...
                    schema = self.resolve_serializer(
                        parameter.type, direction).ref
    ...
    def _get_parameters(self):
        ...
        for key, examples in examples_by_key.items():
            if key in parameters:
                ...
                parameters[key]['examples'].update(
                    build_examples_list(examples))
    ...
    def get_description(self):
        """ override this for custom behaviour """
        action_or_method = getattr(self.view, getattr(
            self.view, 'action', self.method.lower()), None)
    ...
    def get_auth(self):
        ...
        for authenticator in self.view.get_authenticators():
            ...
            if isinstance(scheme.name, str):
                ...
                names, definitions = scheme.name, scheme.get_security_definition(
                    self)
    ...
    def _get_callbacks(self):
        ...
        for callback in self.get_callbacks():
            ...
            for method, decorator in methods.items():
                ...
                if extensions:
                    operation.update(
                        sanitize_specification_extensions(extensions))
                ...
                try:
                    if getattr(self.view, 'lookup_url_kwarg', None) == variable:
                        model_field_name = getattr(
                            self.view, 'lookup_field', variable)
                    ...
                    model_field = follow_model_field_lookup(
    ...
    def _map_serializer_field(self, field, direction, bypass_extensions=False):
        ...
        if has_override(field, 'field'):
            ...
            if is_basic_type(override):
                ...
                schema = self._map_serializer_field(
                    force_instance(override), direction)
        ...
        if isinstance(field, serializers.ManyRelatedField):
            schema = self._map_serializer_field(
                field.child_relation, direction)
        ...
        if isinstance(field, serializers.ReadOnlyField):
            ...
            if is_list_serializer(field.parent):
                model = getattr(getattr(field.parent.parent,
                                'Meta', None), 'model', None)
                source = field.parent.source_attrs
            else:
                model = getattr(
                    getattr(field.parent, 'Meta', None), 'model', None)
        ...
        for v in field.validators:
            if schema_type == 'string':
                ...
                if isinstance(v, validators.MaxValueValidator):
                    ...
                    update_constraint(schema, 'maximum', min,
                                      value, exclusive=True)
                    update_constraint(schema, 'minimum',
                                      max, -value, exclusive=True)
                ...
                if isinstance(v, validators.MaxLengthValidator):
                    update_constraint(schema, 'maxProperties',
                                      min, v.limit_value)
                elif isinstance(v, validators.MinLengthValidator):
                    update_constraint(schema, 'minProperties',
                                      max, v.limit_value)

    def _map_response_type_hint(self, method):
        hint = get_override(method, 'field') or get_type_hints(
            method).get('return')
    ...
    def _get_examples(self, serializer, direction, media_type, status_code=None, extras=None):
        ...
        for example in examples + extras:
            ...
            if not example.media_type:
                example_media_type = media_type if (
                    example in extras) else 'application/json'
            else:
            ...
            if not example.status_codes:
                example_status_codes = (status_code,) if (
                    example in extras) else ('200', '201')
            else:
    ...
    def _get_request_body(self, direction='request'):
        ...
        if isinstance(request_serializer, dict):
            ...
            if schema is None:
                return None
            content = [
                (media_type, schema, self._get_examples(
                    request_serializer, direction, media_type), None)
            ...
            **headers,
            'content': {
                media_type: build_media_type_object(
                    schema,
                    self._get_examples(serializer, direction,
                                       media_type, status_code, examples)
                )
