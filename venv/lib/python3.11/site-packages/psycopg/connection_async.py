...
class AsyncConnection(BaseConnection[Row]):
    ...
    def __init__(
        self,
        pgconn: "PGconn",
        row_factory: AsyncRowFactory[Row] = cast(
            AsyncRowFactory[Row], tuple_row),
    ):
    ...
    # type: ignore[misc] # https://github.com/python/mypy/issues/11004
    @classmethod
    async def connect(
        cls,
        conninfo: str = "",
    ...
    async def notifies(self) -> AsyncGenerator[Notify, None]:
        while True:
            ...
            for pgn in ns:
                n = Notify(pgn.relname.decode(enc),
                           pgn.extra.decode(enc), pgn.be_pid)
                yield n
