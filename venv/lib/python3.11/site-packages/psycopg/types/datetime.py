...
_unpack_timetz = cast(
    Callable[[Buffer], Tuple[int, int]], _struct_timetz.unpack)
...
class DateLoader(Loader):
    ...
    def __init__(self, oid: int, context: Optional[AdaptContext] = None):
        ...
        if ds.startswith(b"I"):  # ISO
            ...
            self._order = self._ORDER_DMY if ds.endswith(
                b"DMY") else self._ORDER_MDY
        else:
    ...
    def load(self, data: Buffer) -> date:
        ...
        try:
            ...
            if s == "infinity" or (s and len(s.split()[0]) > 10):
                raise DataError(
                    f"date too large (after year 10K): {s!r}") from None
            elif s == "-infinity" or "BC" in s:
                raise DataError(
                    f"date too small (before year 1): {s!r}") from None
            else:
...
class TimeBinaryLoader(Loader):
    ...
    def load(self, data: Buffer) -> time:
        ...
        try:
            ...
            raise DataError(
                f"time not supported by Python: hour={h}") from None
...
class TimestampLoader(Loader):
    ...
    def __init__(self, oid: int, context: Optional[AdaptContext] = None):
        ...
        if ds.startswith(b"I"):  # ISO
            ...
            self._order = self._ORDER_PGDM if ds.endswith(
                b"DMY") else self._ORDER_PGMD
    ...
    def load(self, data: Buffer) -> datetime:
        ...
        try:
            ...
            raise _get_timestamp_load_error(
                self.connection, data, ex) from None
...
class TimestampBinaryLoader(Loader):
    ...
    def load(self, data: Buffer) -> datetime:
        ...
        try:
            ...
            if micros <= 0:
                raise DataError(
                    "timestamp too small (before year 1)") from None
            else:
                raise DataError(
                    "timestamp too large (after year 10K)") from None
...
class TimestamptzLoader(Loader):
    ...
    def load(self, data: Buffer) -> datetime:
        ...
        ex: Exception
        try:
            dt = datetime(int(ye), int(mo), int(
                da), int(ho), int(mi), int(se), us, utc)
        ...
        try:
            ...
            if self._timezone:
                ...
                if utcoff:
                    ...
                    try:
                        ts = _pg_datetime_epoch + \
                            timedelta(microseconds=micros + usoff)
                    except OverflowError:
