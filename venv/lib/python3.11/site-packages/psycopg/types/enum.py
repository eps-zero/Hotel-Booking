...
def register_enum(
    ...
    *,
    ...
    if enum is None:
        enum = cast(Type[E], Enum(info.name.title(),
                    info.labels, module=__name__))
    ...
    loader = type(name, (_BaseEnumLoader,), {
                  **attribs, "format": Format.BINARY})
    adapters.register_loader(info.oid, loader)
    ...
    dumper = type(name, (_BaseEnumDumper,), {
                  **attribs, "format": Format.BINARY})
    adapters.register_dumper(info.enum, dumper)
