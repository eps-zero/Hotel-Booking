...
class PGconn:
    ...
    def __init__(self, pgconn_ptr: impl.PGconn_struct):
        ...
        impl.PQsetNoticeReceiver(
            pgconn_ptr, notice_receiver, byref(self._self_ptr))
    ...
    def pgconn_ptr(self) -> Optional[int]:
        ...
        return addressof(self._pgconn_ptr.contents)
    ...
    def exec_(self, command: bytes) -> "PGresult":
        ...
        if not rv:
            raise e.OperationalError(
                f"executing query failed: {error_message(self)}")
        return PGresult(rv)
    ...
    def prepare(
        ...
        if not rv:
            raise e.OperationalError(
                f"preparing query failed: {error_message(self)}")
        return PGresult(rv)
    ...
    def describe_portal(self, name: bytes) -> "PGresult":
        ...
        if not rv:
            raise e.OperationalError(
                f"describe portal failed: {error_message(self)}")
        return PGresult(rv)
...
class PGresult:
    ...
    def pgresult_ptr(self) -> Optional[int]:
        ...
        # type: ignore[attr-defined]
        return addressof(self._pgresult_ptr.contents)
    ...
    def set_attributes(self, descriptions: List[PGresAttDesc]) -> None:
        structs = [
            # type: ignore
            impl.PGresAttDesc_struct(*desc) for desc in descriptions
        ]
        array = (impl.PGresAttDesc_struct *
                 len(structs))(*structs)  # type: ignore
...
class Conninfo:
    ...
    def parse(cls, conninfo: bytes) -> List[ConninfoOption]:
        ...
        errmsg = c_char_p()
        rv = impl.PQconninfoParse(conninfo, byref(
            errmsg))  # type: ignore[arg-type]
        if not rv:
            if not errmsg:
